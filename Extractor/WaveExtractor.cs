...
namespace super_toolbox
{
    public class WaveExtractor : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            Console.WriteLine($"统计提取文件数量: {ExtractedFileCount}");

            int actualExtractedFileCount = Directory.EnumerateFiles(extractedDir, "*", SearchOption.AllDirectories).Count();
            Console.WriteLine($"实际在Extracted文件夹中的子文件数量为: {actualExtractedFileCount}");
        ...
        private async Task ExtractFromFileAsync(
            string filePath, string extractedDir, CancellationToken cancellationToken)
        {
            ...
            foreach (byte[] waveData in ExtractWaveData(fileContent))
            {
                ...
                string tempExtractedFilename = $"{baseFilename}_{ExtractedFileCount}.temp";
                ...
                string finalExtractedFilename = $"{baseFilename}_{ExtractedFileCount}.{detectedExtension}";
