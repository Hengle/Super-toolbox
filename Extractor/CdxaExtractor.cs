    ...
    public class CdxaExtractor : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            foreach (string filePath in files)
            {
                ...
                try
                {
                    await ExtractXaFilesAsync(filePath, extractedDir, cancellationToken);
                }
                catch (Exception ex)
                {
        ...
        private async Task ExtractXaFilesAsync(string filePath, string extractedDir, CancellationToken cancellationToken)
        {
            ...
            foreach (string extractedFilePath in extractedFileNames)
            {
                if (File.Exists(extractedFilePath))
                {
                    try
                    {
                        ...
                        string newFileName = GetUniqueFileName(extractedDir, fileName);
            ...
            {
                OnExtractionFailed($"清理临时目录 {sourceFileSubDir} 失败: {ex.Message}");
            }
        }
        ...
        private string GetUniqueFileName(string directory, string fileName)
        {
            ...
            do
            {
                newFileName = Path.Combine(directory, $"{fileNameWithoutExt}_{counter}{fileExt}");
                counter++;
            }
