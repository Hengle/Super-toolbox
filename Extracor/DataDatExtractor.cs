...
namespace super_toolbox
{
    public class DataDatExtractor : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    Parallel.ForEach(files, new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount }, filePath =>
                    {
                        try
                        {
                            ExtractPMF(filePath, videoDir, counters);
                            ExtractPNG(filePath, imageDir, counters);
                            ExtractAT3(filePath, audioDir, counters);
                        }
                        catch { }
                    });
                }, cancellationToken);
            ...
            Console.WriteLine("提取统计:");
            foreach (var kvp in counters)
            {
                Console.WriteLine($"  - {kvp.Key.ToUpper()}: {kvp.Value} 个文件");
            }
        }

        private void ExtractPMF(string filePath, string outputDir, ConcurrentDictionary<string, int> counters)
        {
            ...
            while (pos < content.Length)
            {
                ...
                if (pmfData.Length > 6)
                {
                    int count = counters.AddOrUpdate("pmf", 1, (k, v) => v + 1);
                    string outPath = Path.Combine(outputDir, $"data{count}.pmf");
        ...
        private void ExtractPNG(string filePath, string outputDir, ConcurrentDictionary<string, int> counters)
        {
            byte[] content = File.ReadAllBytes(filePath);
            ...
            for (int index = 0; index < content.Length;)
            {
                ...
                else
                {
                    ...
                    if (endIdx != -1)
                    {
                        ...
                        {
                            int count = counters.AddOrUpdate("png", 1, (k, v) => v + 1);
                            string outPath = Path.Combine(outputDir, $"data{count}.png");
        ...
        private void ExtractAT3(string filePath, string outputDir, ConcurrentDictionary<string, int> counters)
        {
            byte[] content = File.ReadAllBytes(filePath);
            int waveDataStart = 0;
            while (true)
            {
                ...
                try
                {
                    ...
                    if (hasWave && hasFmt)
                    {
                        ...
                        {
                            int count = counters.AddOrUpdate("at3", 1, (k, v) => v + 1);
                            string outPath = Path.Combine(outputDir, $"data{count}.at3");
