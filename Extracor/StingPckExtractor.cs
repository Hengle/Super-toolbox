...
namespace super_toolbox
{
    public class StingPckExtractor : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            OnCompletion(_extractedFileCount);
        }
        ...
        private async Task ProcessSinglePckFileAsync(string filePath, string outputDir, CancellationToken ct)
        {
            await Task.Run(() =>
            {
                ...
                using (var reader = new ExtendedBinaryReader(ms))
                {
                    ...
                    Interlocked.Add(ref _extractedFileCount, pckFile.FileEntries.Count);

                    foreach (var entry in pckFile.FileEntries)
                    {
                        OnFileExtracted(Path.Combine(pckName, entry.FileName));
                    }
                }
            }, ct);
        }
        ...
        private new void OnFileExtracted(string file) => FileExtracted?.Invoke(this, $"已提取: {file}");
        private void OnCompletion(int count) => ExtractionCompleted?.Invoke(this, count);
    }
}