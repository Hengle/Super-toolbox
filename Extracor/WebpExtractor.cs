...
namespace super_toolbox
{
    public class WebpExtractor : BaseExtractor
    {
        ...
        {
            Console.WriteLine($"提取进度: {_extractedFileCount}/{TotalFilesToExtract}");
        }
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            foreach (string filePath in files)
            {
                ...
                try
                {
                    int extractedCount = await ExtractWebpsFromFileAsync(filePath, extractedDir, cancellationToken);
                    Interlocked.Add(ref _extractedFileCount, extractedCount);
                    OnProgressUpdated();
                }
                catch (Exception ex)
                {
        ...
        private async Task<int> ExtractWebpsFromFileAsync(string filePath, string extractedDir, CancellationToken cancellationToken)
        {
            ...
            foreach (byte[] webpData in ExtractWebpData(fileContent))
            {
                ...
                string extractedFilename = $"{baseFilename}_{fileCount}.webp";
